...
dv_t* dvtable_create()
{
	...
	for (k=0;k<5;k++)
	{
		...
		if (dv->nodeID==id1)
		{
			...
			for (i=0;i<num;i++)
			{
				Node->dv_nbr->dvEntry[i].nodeID = node_array[i];
				if (node_array[i]==Node->dv_nbr->nodeID)
					Node->dv_nbr->dvEntry[i].cost = 0;
				else
					if (node_array[i]==dv->nodeID)
						Node->dv_nbr->dvEntry[i].cost = len;
					else
						Node->dv_nbr->dvEntry[i].cost = INFINITE_COST;
			}
			Node->next = NULL;
			if (lhead == NULL)
				lhead = Node;
    ...
    fclose(fp);
	printf("dv_table create successful!\n");
  return dv;
}
