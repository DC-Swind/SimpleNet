...
int stcp_client_recv(int sockfd, void* buf, unsigned int length)
{
    ...
    while(1){
        ...
        if(length > tcb[sockfd]->usedBufLen){
            ...
            memcpy(tcb[sockfd]->recvBuf,&tcb[sockfd]->recvBuf[length],length);
...
void handle(seg_t* buffer,int src_nodeID)
{
    ...
    case 4:
        ...
        */
        ...
        if (tcb[i]->state == CONNECTED && tcb[i]->usedBufLen + buffer->header.length <= RECEIVE_BUF_SIZE){
            if (tcb[i]->expect_seqNum < buffer->header.seq_num) return;
            if (tcb[i]->expect_seqNum == buffer->header.seq_num){
                pthread_mutex_lock(tcb[i]->bufMutex);
                memcpy(&tcb[i]->recvBuf[tcb[i]->usedBufLen],buffer->data,buffer->header.length);
                tcb[i]->usedBufLen += buffer->header.length;
                        //printf("already receive %d.\n",TCB[index]->usedBufLen);
                tcb[i]->expect_seqNum += 1;
            ...
            shdr->src_port = buffer->header.dest_port;
            shdr->dest_port = buffer->header.src_port;
            shdr->seq_num = 0;
            shdr->ack_num = tcb[i]->expect_seqNum ; //-1 or not
            shdr->length = 0;
            shdr->type = DATAACK;
            shdr->rcv_win =0;
            shdr->checksum = 0;
            if (sip_sendseg(sip_conn,tcb[i]->server_nodeID,sbuf) > 0){
