...
void* cThread_send(void *indext){
    ...
    while(1){
        ...
        if (threads[index].requestgame != '0'){
            ...
            if (send(threads[index].connsocket,(char *)&package,sizeof(struct Package),0)<0){
                //logout(index);
...
void* cThread(void* indext){
    ...
    while(waitLogin){
        ...
        if (stcp_server_send(connsocket,(char *)&acklogin,sizeof(struct Package))){
            logout(index);
